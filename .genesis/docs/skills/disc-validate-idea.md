...
- Após [`discover-business`](disc-discover-business.md) registrar premissas em `assumptions.md` com confiança baixa.
- Após [`define-product`](plan-define-product.md) — premissa "vão usar isso" precisa validar antes de construir.
- Após [`design-business-model`](plan-design-business-model.md) — premissa "vão pagar R$ X" precisa de smoke test ou piloto.
...
A decisão **sempre** gera ADR (via [`create-adr`](plan-create-adr.md)).
...
| ADR | Se decisão de pivot/manter veio do resultado. | Via [`create-adr`](plan-create-adr.md). |
...
➡️ Crie ADR via [`create-adr`](plan-create-adr.md) registrando a decisão.
...
- Agents relevantes:
  - [`ux-researcher`](../../../.claude/agents/plan-ux-researcher.md) — para entrevistas.
  - [`business-mentor`](../../../.claude/agents/plan-business-mentor.md) — para validar hipóteses de monetização.
