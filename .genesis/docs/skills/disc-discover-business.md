...
| Skill anterior | [`init-project`](plan-init-project.md) |
| Skill seguinte | [`design-business-model`](plan-design-business-model.md) |
...
- [ ] [`init-project`](plan-init-project.md) concluído.
...
Você deve revisar cada premissa: a confiança está honesta? Se há premissa com confiança 1-2 e impacto alto/fatal, **considere invocar [`validate-idea`](disc-validate-idea.md) antes de avançar**.

### 5. Decida se precisa de ADR

Se uma decisão importante surgir nesta skill (ex.: "modelo principal de receita será comissão de marketplace, não assinatura"), peça para criar ADR via [`create-adr`](plan-create-adr.md). Não é obrigatório agora — pode ser feito depois, mas registrar cedo evita debate repetido.

### 6. Atualize `PROJECT_STATE.md`

A IA marca Fases 2 e 3 como concluídas e indica próxima skill ([`design-business-model`](plan-design-business-model.md) ou [`define-product`](plan-define-product.md), dependendo do desbalanceamento — geralmente `design-business-model` se a monetização ainda está nebulosa).
...
- A-0001: "Donas de escolas pequenas pagam R$ 200/mês por gestão integrada." Confiança 2, impacto alto. **Precisa validação** via [`validate-idea`](disc-validate-idea.md).
...
**Próximo passo sugerido:** invocar [`design-business-model`](plan-design-business-model.md) para detalhar planos e pricing antes de definir o MVP.
...
➡️ **[`design-business-model`](plan-design-business-model.md)** — detalhar planos, pricing, GTM antes do MVP.

ou

➡️ **[`define-product`](plan-define-product.md)** — se monetização já está clara, ir direto para MVP.
...
- Agents relevantes:
  - [`business-mentor`](../../../.claude/agents/plan-business-mentor.md) — invoke para revisar premissas e unit economics.
  - [`ux-researcher`](../../../.claude/agents/plan-ux-researcher.md) — para refinar personas.
  - [`monetization-strategist`](../../../.claude/agents/plan-monetization-strategist.md) — para revisar fontes de receita.
