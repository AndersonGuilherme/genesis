...
| Skill anterior | [`define-product`](plan-define-product.md) |
| Skill seguinte | [`choose-stack`](plan-choose-stack.md) |
...
- Não decide microserviços vs. monolito modular (decisão arquitetural, vai em [`design-architecture`](plan-design-architecture.md)).

## Quando você deve invocar

- Após [`define-product`](plan-define-product.md) ter consolidado MVP.
- Antes de [`choose-stack`](plan-choose-stack.md) — a stack depende dos módulos.
...
A IA atualiza e sugere [`choose-stack`](plan-choose-stack.md).
...
🚫 **Decidir microserviços agora.** Fase 5 trata de módulos lógicos. Decidir se vira microserviço é decisão arquitetural — vai em ADR durante [`design-architecture`](plan-design-architecture.md).
...
➡️ **[`choose-stack`](plan-choose-stack.md)** — agora que módulos estão definidos, avaliar tecnologias adequadas.
...
- Agents relevantes:
  - [`software-architect`](../../../.claude/agents/plan-software-architect.md) — pode revisar fronteiras.
  - [`domain-modeler`](../../../.claude/agents/plan-domain-modeler.md) — para validar entidades por módulo.
