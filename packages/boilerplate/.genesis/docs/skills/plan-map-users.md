...
| Skill anterior | [`discover-business`](disc-discover-business.md) ou [`define-product`](plan-define-product.md) |
| Skill seguinte | volta para o fluxo principal (frequentemente [`design-architecture`](plan-design-architecture.md)) |
...
- Após [`discover-business`](disc-discover-business.md) ter mapeado público em alto nível.
- **Antes** de [`design-architecture`](plan-design-architecture.md), para alimentar `auth-strategy.md`.
...
Volte ao fluxo principal — geralmente [`design-architecture`](plan-design-architecture.md) (que usa o output desta skill para refinar `auth-strategy.md`).
...
| ADR | Se decisão grande surgir (ex.: "uma conta por usuário, múltiplos tenants" vs. "conta separada por tenant"). | Via [`create-adr`](plan-create-adr.md). |
...
➡️ **[`design-architecture`](plan-design-architecture.md)** — usar a matriz para definir `auth-strategy.md` em detalhe.

ou (caso esteja em fase de spec):

➡️ **[`define-module-spec`](plan-define-module-spec.md)** — usar as permissões nesta skill para preencher `business-rules.md` do módulo.
...
- Agents relevantes:
  - [`security-reviewer`](../../../.claude/agents/plan-security-reviewer.md) — para revisar matriz e LGPD de menores.
  - [`ux-researcher`](../../../.claude/agents/plan-ux-researcher.md) — para refinar diferenças entre usuário/comprador/decisor.
